 ...
 * Returns: Nothing.
 */
static void print_edge(const void *from, const void *to, const char *port, const char *label,
                       const char *options)
{
    if (port) {
 ...
 * Returns: Nothing.
 */
static void print_head_node(const array_1d *a)
{
    printf("\tm%04lx [shape=record label=\"kill\\n%04lx|low\\n%d|high\\n%d"
	   "|array_size\\n%d|<v>values\\n%04lx\"]\n", PTR2ADDR(a),
	   PTR2ADDR(a->kill_func), a->low, a->high, a->array_size,
	   PTR2ADDR(a->values));
}

void print_values(const array_1d *a)
{
    printf("\tm%04lx [shape=record label=\"", PTR2ADDR(a->values));
    for (int i=0; i < a->array_size; i++) {
...
static void print_head_edge(const array_1d *a)
{
    printf("\tm%04lx:v -> ", PTR2ADDR(a));
    if (a->values == NULL) {
	printf("NULL");
    } else {
...
static void print_value_nodes(const array_1d *a, inspect_callback print_func)
{
    for (int i=0; i <= a->array_size; i++) {
	if (a->values[i] != NULL) {
	    printf("\tm%04lx [label=\"", PTR2ADDR(a->values[i]));
	    if (print_func != NULL) {
...
static void print_value_edges(const array_1d *a)
{
    for (int i=0; i < a->array_size; i++) {
	...
	sprintf(port, "%02d", i);
	if (a->kill_func) {
	    print_edge(a->values, a->values[i], port, port, "color=red");
	} else {
	    print_edge(a->values, a->values[i], port, port, "color=red style=dashed");
	}
    }
}
...
/**
 * array_1d_print_internal() - Print the arrays internal structure in dot format.
 * @a: Array to inspect.
			     ...
			     bool outermost)
{
    ...
    if (outermost) {
        ...
        // ...otherwise start a subgraph
        printf("\tsubgraph cluster_array_1d_%d {\n", graph_number);
    }

    // Specify default shape and fontname
    printf("\tnode [shape=rectangle fontname=\"Courier New\"]\n");
    ...
    if (desc != NULL) {
        ...
        if (outermost) {
            ...
            printf("\tcluster_array_1d_%d_description [label=\"%s\"]\n", graph_number, escaped);
        }
        // Return the memory used by the escaped string
        free(escaped);
    }

    // Output the NULL node
    printf("\tNULL\n");
    ...
    // Print nodes for each value memory block
    print_value_nodes(a, print_func);

    // Print edges from each value pointer to payload memory
    print_value_edges(a);

    if (!outermost) {
        // For inner datatypes, indent the termination brace, thus
        // indenting everything except the digraph open/close braces
        printf("\t");
    }

    // Termination of graph or subgraph
    printf("}\n");
}
